...
/*********************************************************
 * MMI_atLeastOneWithFreshMilk
 *
 *	[iIcon] va da 0 a MMI_getCount()-1
 *
 *	ritorna 1 se tutte le selezioni incluse nell'icona sono selezioni con latte fresco
 */
 function  MMI_atLeastOneWithFreshMilk (iIcon)
 {
	 if (iIcon < 0) return 0;
	 if (iIcon >= MMI_getCount()) return 0;
	 ...
	 if (selNum != 0)
	 {
		 return rhea.selection_getBySelNumber(selNum).withFreshMilk;
	 }
	 
	 var allWithFreshMilk = 1;
	 for (var grinder=0; grinder<2; grinder++)
	 {
		 for (var cup=0; cup<3; cup++)
		 {
			 for (var shotType=0; shotType<2; shotType++)
			 {
				 ...
				 if (selNum > 0)
				 {
					 if (rhea.selection_getBySelNumber(selNum).withFreshMilk == 0)
						 allWithFreshMilk = 0;
				 }
			 }
		 }
