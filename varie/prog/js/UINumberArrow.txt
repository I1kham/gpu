 ...
 *			opzionale data-da3bit="4|8|9|16|n"	=> legge 4,8,9,16,32 bit dal da3, default 16.
													NEl caso di 9 bit, è necessaria anche l'opzione data-da3bit9
 *          opzionale data-da3bitpos="x"    => la posizione nel byte indicato in "data-da3" a partire dal quale scrivere [ valori: 0%7 ]
 ...
 *			opzionale data-soloPari				=> default = 0, se ==1 allora il controllo visualizza solo numeri pari
 *			opzionale data-readonly				=> default = 0, se ==1 allora il controllo e' in sola lettura
  */
...
function UINumber (parentID, childNum, node)
{
	...
	this.data_da3bitpos = parseInt(UIUtils_getAttributeOrDefault(node, "data-da3bitpos", "0"));

	this.soloPari = false;
	if (1 == parseInt(UIUtils_getAttributeOrDefault(node, "data-soloPari", "0")))
		this.soloPari = true;

	this.readonly = false;
	if (1 == parseInt(UIUtils_getAttributeOrDefault(node, "data-readonly", "0")))
		this.readonly = true;
...
UINumber.prototype.priv_getHTMLForAFigure = function(i)
{
	...
	if (this.readonly == false)
		html += "	<div style='position:absolute; top:3px; left:17px; z-index:22;'><img src='img/UINumberArrowUp-grey.png' draggable='false'></div>"
				+"	<div style='position:absolute; top:76px; left:17px; z-index:22;'><img src='img/UINumberArrowDown-grey.png' draggable='false'></div>";
				
	html +=  "	<div id='" +idClicker +"' class='UINumber3Clicker'>&nbsp;</div>"
			+"</div>";
	return html;
}
...
UINumber.prototype.saveToDA3 = function(da3)		
{
	...
	if (loc >= 0) 
	{
		...
		switch (this.da3bit)
		{
			...
			case 16: 	da3.write16(loc, value); break;
			...
			default:
				var val = (value << this.data_da3bitpos);
				var mask = parseInt(~(~(0xFF << this.da3bit) << this.data_da3bitpos) & 0xFF);
				var data = (da3.read8(loc) & mask);
				da3.write8(loc, val | data);
				break;	
		}		
	}
}
	...
	switch (this.da3bit)
	{
		...
		case 9:
			...
			if ((v2 & mask) != 0)
				value += 256;
			break;
        case 16: 	value = da3.read16(loc); break;
		case 32:	value = da3.read32(loc); break;
		default:
			value = ((da3.read8(loc) >> this.data_da3bitpos) & (0xFF >> (8 - this.da3bit)));
			break;
	}
...
UINumber.prototype.priv_calcNewValueWhenThereIsAForbiddenList = function (curValue, newValue, bVolevoIncrementare)
{
	var numList = this.forbiddenNumberList.split(",");
		...
		if (timeElapsedMSec <650 && xdiff <60 &&ydiff<40)
		{
			...
			else
			{
				...
				if (me.forbiddenNumberList != "")
					me.setValue( me.priv_calcNewValueWhenThereIsAForbiddenList (curValue, newValue, bVolevoIncrementare));
				else
...
UINumber.prototype.setValue = function(v)
{
	...
	for (var i=0; i<this.numCifre; i++)
	{
		...
		if (this.readonly)
			dStrip.innerHTML = "<p style='color:#888'>" +num +"</p>";
		else
