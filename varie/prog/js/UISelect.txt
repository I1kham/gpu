...
UISelect.prototype.priv_buildHTML = function(node)
{
	...
	var cssColor = "";
	if (this.readonly)
		cssColor = "style='color:#888'";
	for (var i=0; i<nOptions; i++)
	{
		var rowID = this.id +"_row" +i;
		html += "<tr id='" +rowID +"' style='display:none' valign='middle'><td " +cssColor +">" +this.optionCaption[i] +"</td></tr>";
	}
	html += "</table></div>";
	
	if (this.readonly == false)
		html += "<div style='position:absolute; top:0; right:0' class='UISelectBtn'>...</div></div>";
...
UISelect.prototype.priv_openSelectPanel = function()
{
	...
	var html = "";
	var ulOpener = "<ul>";
	if (this.optionValue.length > 10)
		ulOpener = "<ul class='multiColumn'>";
	for (var i=0; i<this.optionValue.length; i++)	
	{
		if (i % 10 == 0)
		{
