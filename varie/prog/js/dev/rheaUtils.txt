...
function rheaShowElem_TD (elem)		{ rheaSetDisplayMode(elem, "table-cell");}
...
//**************************************************
function rheaFormatPrice (price, numDecimalIN, decimalSeparatorChar) 
{
	let numDecimal= parseInt(numDecimalIN);
	let p = price.toString();
	if (0 == numDecimal)
		return p;
	
	var minLen = numDecimal+1;
	while (p.length < minLen)
		p = "0" +p;
	
	var n = p.length;
	return p.substr(0, n-numDecimal) +decimalSeparatorChar + p.substr(n-numDecimal);
}