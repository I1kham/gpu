 ...
 *			opzionale data-da3bit="4|8|16|n"	=> legge 4 o 8 bit dal da3, default 8
 *          opzionale data-da3bitpos="x"    => la posizione nel byte indicato in "data-da3" a partire dal quale scrivere
...
UIOption.prototype.priv_buildHTML = function ()
{
	...
	for (var i=0; i<nOptions; i++)
	{
		...
		var tdID = this.id +"_optTD" +i;
		var css = "UIButton";
		html += "<td id='" +tdID +"' width='" +cellSize +"%'><div id='" +btnID +"' class='" +css +"' data-optnum='" +i +"'><p>" +this.optionCaption[i] +"</p></div></td>";
	}
	html += "</tr></table>";
	node.innerHTML = html;
}
...
UIOption.prototype.unhideAllOptions = function ()
{
	const nOptions = this.optionValue.length;
	for (var i=0; i<nOptions; i++)
	{
		var tdID = this.id +"_optTD" +i;
		rheaShowElem_TD(rheaGetElemByID(tdID));
	}
}

UIOption.prototype.hideOptionByValue = function (optValue)
{
	const nOptions = this.optionValue.length;
	for (var i=0; i<nOptions; i++)	
	{
		if (this.optionValue[i] == optValue)
		{
			var tdID = this.id +"_optTD" +i;
			rheaHideElem(rheaGetElemByID(tdID));
			return;
		}
	}
}
...
{ 
	...
	if (loc >= 0) 
	{
        ...
        case 8:     this.selectOptionByValue (da3.read8(loc));  break;
		case 16:	this.selectOptionByValue (da3.read16(loc)); break;
		case 32:	this.selectOptionByValue (da3.read32(loc)); break;

		default:	
	        {
	            var val = ((da3.read8(loc) >> this.data_da3bitpos) & (0xFF >> this.da3bit));
...
{ 
	...
	if (loc >= 0) 
	{
		...
		case 8:	    da3.write8(loc, this.getSelectedOptionValue()); break;
		case 16:	da3.write16(loc, this.getSelectedOptionValue()); break;
		case 32:	da3.write32(loc, this.getSelectedOptionValue()); break;
        default:
            var val = parseInt(this.getSelectedOptionValue() << this.data_da3bitpos);
